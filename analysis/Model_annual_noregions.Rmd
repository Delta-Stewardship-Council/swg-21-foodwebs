...
definations_data = focaldata %>% 
  ...
  select(Variable, Range, Years_missing, Defination) 
...
write.csv(definations_data,
          "fig_output/definitions tabel.csv",
          row.names = FALSE)
```
...
```{r}
coef_tabler(modfit1, modfit1_dtr, name="")%>%
  write.csv("fig_output/annual noregion coefficients.csv", row.names = FALSE)
```
