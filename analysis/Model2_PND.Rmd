...
psych::pairs.panels(dplyr::select(fd, year, flow, temp, secchi, chla, hcope, clad, pcope, mysid, estfish, sside),lm = T)
```
...
# labels2 <- createLabels(modfit2, cnames)
# labels3 <- createLabels(modfit3, cnames)
#labels4 <- createLabels(modfit4, cnames)
# 
# test_1 <- myLavaanPlot(model=modfit1, labels=labels1, 
# 						 node_options=list(shape="box", fontname="Helvetica"), 
# 						 coefs=TRUE, stand=TRUE, covs=FALSE, sig=0.05, 
# 						 width=c("regress","latent"),
# 						 color=c("regress","latent"))

run_myLavaanPlot = function(model, resolution){
  model_fit <- sem(model, data=fd)
  labels <- createLabels(model_fit, cnames)
  out_plot <- myLavaanPlot(model=model_fit, labels=labels, 
						 node_options=list(shape="box", fontname="Helvetica"), 
						 coefs=TRUE, stand=TRUE, covs=FALSE, sig=0.05, 
						 width=c("regress","latent"),
						 color=c("regress","latent"))
...
plots <-  map(model_list, ~run_myLavaanPlot(.x, 2000))
figure_1 = ggarrange(plotlist=plots, labels = "auto")  

ggsave('./analysis/filename.png',figure_1,width=24,height=10, dpi = 600)
