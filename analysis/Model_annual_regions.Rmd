...
plot1=select(focaldata,region,year,varnames) %>% rbind(focaldata_we2) %>% 
  ...
  mutate(Diagramname=gsub("[\r\\\\\\\n]"," ",.data$Diagramname),
         ...
         region=factor(region,levels=c("Whole estuary", regionorder)))
...
plot_grobs <- map(list(sem_annual_noregions, figW, figN, figS),
...
#detrended
plot_grobs <- map(list(sem_annual_noregions_dtr, figWdt, figNdt, figSdt),
                  ~convert_html_to_grob(.x, 2000))
...
bind_rows(
  ...
  write.csv("fig_output/annual region coefficients.csv", row.names = FALSE)
```
