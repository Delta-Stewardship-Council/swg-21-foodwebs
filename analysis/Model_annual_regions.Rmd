...
#ggplot(fd,aes(x=year, y=flow, color=region)) + geom_line()
```
...
psych::pairs.panels(dplyr::select(fd, year, flow, temp, secchi, chla, hcope, clad, pcope, mysid, estfish, sside, clams, dophos),lm = T, bg=c("red","yellow","blue")[factor(fd$region)],pch=21)
```

### SEM model

```{r}
modnorth='zoop=~hcope+clad+mysid+pcope
        fish=~estfish+estfish_bsmt+estfish_bsot
        zoop~sside+chla+clams
        chla~clams+flow
        fish~zoop+flow+sside
'
modfitnorth=sem(modnorth, data=filter(fd,region=="North"))
...
```{r}
#south has no clams
modsouth='zoop=~hcope+clad+mysid+pcope
        fish=~estfish+estfish_bsmt+estfish_bsot
        zoop~sside+chla
        chla~flow
        fish~zoop+flow+sside
'
modfitsouth=sem(modsouth, data=filter(fd,region=="South"))
...
```{r}
#west has no ssides
modwest='zoop=~hcope+clad+mysid+pcope
        fish=~estfish+estfish_bsmt+estfish_bsot
        zoop~chla+clams
        chla~clams+flow
        fish~zoop+flow
'
modfitwest=sem(modwest, data=filter(fd,region=="West"))
