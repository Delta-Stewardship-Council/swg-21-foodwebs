...
cnames=rbind(cnames,data.frame(Longname = c("Total zooplankton biomass",
                               ...
                               Color=c("black","black","#ED7D31","#ED7D31","#7030A0",
                                       "#7030A0","#4472C4"),
                               Defination = c("summed zooplankton biomass",
...
definations_data = focaldata %>% 
  ...
  left_join(select(cnames, Longname, Shortname, Defination), by = "Shortname") %>% 
  group_by(Shortname, Longname, Defination) %>% 
  ...
  select(Variable, Months_missing, Defination) %>% 
  filter(Variable != "Month",
         Variable != "Year")

# Do a little formatting of the table
knitr::kable(definations_data, col.names = c("Variable",
                                             "Years (missing years)",
                                             "Defination"))
# Write it as a csv
write.csv(definations_data,
