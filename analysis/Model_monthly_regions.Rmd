...
#FAR WEST
plot_far_west = generateInputDf(fit=modfitFW, reference_df=cnameslag) %>%
  getUpperTrophicCoord() %>%
  createGraph(fit=modfitFW) %>%
  DiagrammeR::render_graph(title="Far West")

#WEST
plot_west = generateInputDf(fit=modfitW, reference_df=cnameslag) %>%
  getUpperTrophicCoord() %>%
  createGraph(fit=modfitW) %>%
  DiagrammeR::render_graph(title="West")

#NORTH
plot_north = generateInputDf(fit=modfitN, reference_df=cnameslag) %>%
  getUpperTrophicCoord() %>%
  createGraph(fit=modfitN) %>%
  DiagrammeR::render_graph(title="North")

#SOUTH
plot_south = generateInputDf(fit=modfitS, reference_df=cnameslag) %>%
  getUpperTrophicCoord() %>%
  createGraph(fit=modfitS) %>%
  DiagrammeR::render_graph(title="South")


```
Save the plots to one figure

```{r}

# Libraries needed for this
# Move to top once finalized
library(purrr)
...
plot_list = list(plot_far_west,
                 plot_west,
                 plot_north,
                 plot_south)

plots <-  map(plot_list, ~convert_html_to_grob(.x, 2000))
figure_1 = ggarrange(plotlist=plots, labels = "auto") %>% 
  annotate_figure(top = text_grob("Monthly Regional Models", 
                                  color = "black",
                                  face = "bold",
                                  size = 14))

ggsave('./analysis/test_figure.png',figure_1,width=6,height=3, dpi = 300)
...
#FAR WEST
generateInputDf(fit=modfitFW, reference_df=cnameslag) %>%
  getLowerTrophicCoord() %>%
  createGraph(fit=modfitFW) %>%
  DiagrammeR::render_graph(title="Far West")

#WEST
generateInputDf(fit=modfitW, reference_df=cnameslag) %>%
  getLowerTrophicCoord() %>%
  createGraph(fit=modfitW) %>%
  DiagrammeR::render_graph(title="West")

#NORTH
generateInputDf(fit=modfitN, reference_df=cnameslag) %>%
  getLowerTrophicCoord() %>%
  createGraph(fit=modfitN) %>%
  DiagrammeR::render_graph(title="North")

#SOUTH
generateInputDf(fit=modfitS, reference_df=cnameslag) %>%
  getLowerTrophicCoord() %>%
  createGraph(fit=modfitS) %>%
  DiagrammeR::render_graph(title="South")
...
#FAR WEST
generateInputDf(fit=modfitFW, reference_df=cnameslag) %>%
  getZoopCoord(region="Far West") %>%
  createGraph(fit=modfitFW) %>%
  DiagrammeR::render_graph(title="Far West")

#WEST
generateInputDf(fit=modfitW, reference_df=cnameslag) %>%
  getZoopCoord(region="West") %>%
  createGraph(fit=modfitW) %>%
  DiagrammeR::render_graph(title="West")

#NORTH
generateInputDf(fit=modfitN, reference_df=cnameslag) %>%
  getZoopCoord(region="North") %>%
  createGraph(fit=modfitN) %>%
  DiagrammeR::render_graph(title="North")

#SOUTH
generateInputDf(fit=modfitS, reference_df=cnameslag) %>%
  getZoopCoord(region="South") %>%
  createGraph(fit=modfitS) %>%
  DiagrammeR::render_graph(title="South")
