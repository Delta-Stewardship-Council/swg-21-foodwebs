...
```{r}
# add_logs = function(inputs){
#   output = map_dbl(inputs, ~exp(.x)) %>% 
#     sum() %>% 
#     log()
#   return(output)
# }
...
# fd = fd %>% 
#   rowwise() %>% 
#   mutate(rep_zoop_c = add_logs(c(hcope_c,clad_c,mysid_c)),
#          rep_zoop_m =  add_logs(c(hcope,clad,mysid)),
#          rep_zoop_e =  add_logs(c(hcope_e,clad_e,mysid_e)),
#          all_zoop_c =  add_logs(c(hcope_c,clad_c,mysid_c,pcope_c,rotif_c)),
#          all_zoop_m =  add_logs(c(hcope,clad,mysid,pcope,rotif_m)),
#          all_zoop_e =  add_logs(c(hcope_e,clad_e,mysid_e,pcope_e,rotif_e)),
#          all_fish =  add_logs(c(estfish_bsmt,estfish_bsot)))
#   
# fd_dtr = fd_dtr %>% 
#   rowwise() %>% 
#   mutate(rep_zoop_c = add_logs(c(hcope_c,clad_c,mysid_c)),
...
met_compare = ggplot(data = models,
                     aes(x = model_name,
                         y = metr,
                         color = type)) +
  theme_classic(base_size = 20) +
  labs(title = "SEM", y = metric)+
  theme(axis.text.x = element_text(angle = 90)) +
  geom_point()
print(met_compare)

lm_data_ranked = models %>% 
   mutate(model_name = forcats::fct_reorder(model_name, lm_AIC))

lm_compare = ggplot(data = lm_data_ranked ,
                     aes(x = model_name,
                         y = lm_AIC,
                         color = type)) +
  theme_classic(base_size = 20) +
  labs(title = "Linear Model", y = metric)+
  theme(axis.text.x = element_text(angle = 90)) +
  geom_point()
print(lm_compare)

AIC_compare = ggplot(data = models,
                     aes(x = lm_AIC,
                         y = metr,
                         color = type)) +
  theme_classic(base_size = 20) +
  labs(y = paste0("SEM ", metric))+
  theme(axis.text.x = element_text(angle = 90)) +
  geom_point()
print(AIC_compare)
```
