...
zoop_energy_data<-read.csv("data/zoop_energy_data/zoop_energy_density_dry.csv") %>% 
  ...
  rename(energyj_per_g_wet_mass = energy_density_j_per_g_dry_mass_mean)
...
zoop_mass_group<-read_excel("data/data_in/Zoop Categories for SEM.xlsx", na = "NA")%>%
  ...
  left_join(zoop_energy_data, by = "Group")
...
zoop_data_mass<-zoop_data%>%
  ...
  mutate(BPUE=CPUE*Carbon_mass_micrograms,
         Group=if_else(Order=="Mysida", "Mysida", Group))%>%
...
length(setdiff(unique(zoop_data_mass$Station), unique(zoop_data_annual$Station)))==6
...
zoop_data_annual_final_noregions
...
zoop_data_annual_final_regions
