...
```{r}
zoop_energy_data<-read.csv("data/zoop_energy_data/zoop_energy_density_dry.csv") %>% 
  mutate(Group = str_replace(group, " Copepoda", "_Copepods")) %>% 
  select(Group, energy_density_j_per_g_dry_mass_mean) %>% 
  rename(energyj_per_g_dry_mass = energy_density_j_per_g_dry_mass_mean)
...
zoop_mass_group<-read_excel("data/data_in/Zoop Categories for SEM.xlsx", na = "NA")%>%
  ...
  full_join(zoop_energy_data, by = "Group")%>%
  mutate(Taxlifestage=if_else(Group=="Mysids", "Mysida", Taxlifestage))%>%
  select(Group, Taxlifestage, Carbon_mass_micrograms, energyj_per_g_dry_mass)
...
zoop_data_mass<-zoop_data%>%
  ...
  mutate(BPUE=if_else(is.na(BPUE), BPUE_mysid, BPUE),
         JPUE=(BPUE/1000)*energyj_per_g_dry_mass)
```
