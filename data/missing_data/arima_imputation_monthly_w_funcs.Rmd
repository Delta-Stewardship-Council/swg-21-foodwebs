...
# data wrangling
## import data
This is data generated from the "nutrient_data_monthly.Rmd" script written by Sarah Perry (1-13-2022; 2-24-2022)
```{r}
missingdat_Temperature <- read_csv("data/missing_data/missing_data/missingdat_edi_Temperature.csv") %>% 
  mutate(Date=lubridate::as_date(MonthYear)) %>% 
  select(Date, Analyte:P8) %>%  # rearrange df with date, no time
  arrange(Date) %>% 
  filter(Date> "1994-12-31") # Must filter starting in 1995 for monthly models!
...
temp_ts <- create_station_ts(temp_D28A, 'D28A', monthly = TRUE)
...
df_ts <- create_ts_df(temp_table, model_params, log_trans = TRUE, monthly = TRUE)
# model_fits <- eval_fit(temp_table, model_params, monthly = TRUE)
```
...
## export df
```{r}
df_ts %>% write_csv("data/missing_data/imputed_data/monthly/temperature_edi_imputed_data.csv")
```
...
```{r}
missingdat_Chlorophyll <- read_csv("data/missing_data/missing_data/missingdat_edi_Chlorophyll.csv") %>% 
  mutate(Date=lubridate::as_date(MonthYear)) %>% 
  select(Date, Analyte:P8) %>%  # rearrange df with date, no time
  arrange(Date)
...
chl_ts <- create_station_ts(chl_D28A, 'D28A', monthly = TRUE)
...
df_ts <- create_ts_df(chl_table, model_params, log_trans = TRUE, monthly = TRUE)
# model_fit <- eval_fit(chl_table, model_params, monthly = TRUE)
```
...
## export df
```{r}
df_ts %>% write_csv("data/missing_data/imputed_data/monthly/chlorophyll_edi_imputed_data.csv")
```
...
This is data generated from the "nutrient_data_monthly.Rmd" script written by Sarah Perry (1-13-2022)
```{r}
missingdat_Ammonia <- read_csv("data/missing_data/missing_data/missingdat_edi_DissAmmonia.csv") %>%  
  mutate(Date=lubridate::as_date(MonthYear)) %>% 
  select(Date, Analyte:P8) %>%  # rearrange df with date, no time
  arrange(Date)
...
amm_ts <- create_station_ts(amm_D28A, 'D28A', monthly = TRUE)
...
df_ts <- create_ts_df(amm_table, model_params, log_trans = TRUE, monthly = TRUE)
# model_fits <- eval_fit(amm_table, model_params, monthly = TRUE)
```
...
## export df
```{r}
df_ts %>% write_csv("data/missing_data/imputed_data/monthly/diss_ammonia_edi_imputed_data.csv")
```
...
This is data generated from the "nutrient_data_monthly.Rmd" script written by Sarah Perry (1-13-2022)
```{r}
missingdat_Nit <- read_csv("data/missing_data/missing_data/missingdat_edi_DissNitrateNitrite.csv") %>%  
  mutate(Date=lubridate::as_date(MonthYear)) %>% 
  select(Date, Analyte:P8) %>%  # rearrange df with date, no time
  arrange(Date)

# remove outliers (added in by RL code b/c of high RLs. future code should keep those NAs)
missingdat_Nit$D28A[missingdat_Nit$D28A >= 15] <- NA
missingdat_Nit$D7[missingdat_Nit$D7 >= 7] <- NA
missingdat_Nit$D4[missingdat_Nit$D4 >= 0.5] <- NA
missingdat_Nit$D6[missingdat_Nit$D6 >= 3] <- NA
missingdat_Nit$D41[missingdat_Nit$D41 >= 2] <- NA
...
nit_ts <- create_station_ts(nit_D28A, 'D28A', monthly = TRUE)
...
df_ts <- create_ts_df(nit_table, model_params, log_trans = TRUE, monthly = TRUE)
# model_fits <- eval_fit(nit_table, model_params, monthly = TRUE)
```
...
## export df
```{r}
df_ts %>% write_csv("data/missing_data/imputed_data/monthly/diss_nitrate_nitrite_edi_imputed_data.csv")
```
...
This is data generated from the "nutrient_data_monthly.Rmd" script written by Sarah Perry (1-13-2022)
```{r}
missingdat_phos <- read_csv("data/missing_data/missing_data/missingdat_edi_DissOrthophos.csv") %>%  
  mutate(Date=lubridate::as_date(MonthYear)) %>% 
  select(Date, Analyte:P8) %>%  # rearrange df with date, no time
  arrange(Date)
# str(missingdat_phos)
...
```{r}
# visualize missing data points - 7 missing phos data points
...
phos_ts <- create_station_ts(phos_D28A, 'D28A', monthly = TRUE)
...
plt_obs_and_pred(phos_D28A, phos_ts, phos_forecast, 'Diss phosrate/phosrite (ug/L)')
...
df_ts <- create_ts_df(phos_table, model_params, log_trans = TRUE, monthly = TRUE)
# model_fits <- eval_fit(phos_table, model_params, monthly = TRUE)
```
...
## export df
```{r}
df_ts %>% write_csv("data/missing_data/imputed_data/monthly/diss_orthophos_edi_imputed_data.csv")
```
...
This is data generated from the "nutrient_data_monthly.Rmd" script written by Sarah Perry (1-13-2022)
```{r}
missingdat_Secchi <- read_csv("data/missing_data/missing_data/missingdat_edi_Secchi.csv") %>%  
  mutate(Date=lubridate::as_date(MonthYear)) %>% 
  select(Date, Analyte:P8) %>%  # rearrange df with date, no time
  arrange(Date) %>% 
...
## export df
```{r}
Time_series_imp2 %>% write_csv("data/missing_data/imputed_data/monthly/secchi_edi_imputed_data.csv")
```
...
temp_imputed <- read_csv("data/missing_data/imputed_data/monthly/temperature_edi_imputed_data.csv") %>% 
  mutate(Analyte="Temperature")
chl_imputed <- read_csv("data/missing_data/imputed_data/monthly/chlorophyll_edi_imputed_data.csv")%>% 
  mutate(Analyte="Chlorophyll")
amm_imputed <- read_csv("data/missing_data/imputed_data/monthly/diss_ammonia_edi_imputed_data.csv")%>% 
  mutate(Analyte="DissAmmonia")
nitr_imputed <- read_csv("data/missing_data/imputed_data/monthly/diss_nitrate_nitrite_edi_imputed_data.csv")%>% 
  mutate(Analyte="DissNitrateNitrite")
ophos_imputed <- read_csv("data/missing_data/imputed_data/monthly/diss_orthophos_edi_imputed_data.csv")%>% 
  mutate(Analyte="DissOrthophos")
secchi_imputed <- read_csv("data/missing_data/imputed_data/monthly/secchi_edi_imputed_data.csv")%>% 
  mutate(Analyte="Secchi")

# combine and pivot
monthly_imputed <- bind_rows(temp_imputed, chl_imputed, amm_imputed, nitr_imputed, ophos_imputed, secchi_imputed)
View(monthly_imputed)
summary(monthly_imputed) # only 50 remaining NAs instead of 323 NAs

monthly_imputed_wide <- monthly_imputed %>% 
...
station_region <- read_csv("data/missing_data/regions_monthly.csv")

# join them to region
monthly_imputed_wide_region <- monthly_imputed_wide %>% 
...
monthly_imputed_wide_region$Month <- lubridate::month(monthly_imputed_wide_region$Date) 
monthly_imputed_wide_region$Year <- lubridate::year(monthly_imputed_wide_region$Date)
View(monthly_imputed_wide_region)

# polish the data to look like original dataframe
monthly_imputed_export_region <- monthly_imputed_wide_region %>% 
...
monthly_imputed_export_noregions <- monthly_imputed_wide_region %>% 
...
monthly_imputed_export_region %>% write_csv("data/missing_data/imputed_data/nutrient_data_monthly_regions_imputed.csv")
monthly_imputed_export_noregions %>% write_csv("data/missing_data/imputed_data/nutrient_data_monthly_noregions_imputed.csv")

# 2 - Tanya's model folder
monthly_imputed_export_region %>% write_csv("data/monthly_averages/nutrient_data_monthly_regions_imputed.csv")
monthly_imputed_export_noregions %>% write_csv("data/monthly_averages/nutrient_data_monthly_noregions_imputed.csv")
...
```{r}
# check dwq and edi are the same
df_dwq <- read_csv('data/missing_data/imputed_data/nutrient_data_dwq_monthly_regions_imputed.csv')
df_edi <- read_csv('data/missing_data/imputed_data/nutrient_data_monthly_regions_imputed.csv')

all.equal(df_dwq, df_edi)
```

# compare monthly vs. yearly imputed values. diff?
```{r}
temp_monthly <- read_csv("data/missing_data/imputed_data/monthly/temperature_imputed_monthly_data.csv") %>% 
  select(Date, Station, Imputed_values, Imputed) %>% 
  rename(Imputed_monthly = Imputed_values)
temp_yearly <- read_csv("data/missing_data/imputed_data/yearly/temperature_imputed_yearly_data.csv")%>% 
  select(Date, Station, Imputed_values, Imputed) %>% 
  rename(Imputed_yearly = Imputed_values)

temp_all_imputed <- temp_yearly %>% 
  full_join(temp_monthly) %>% 
  filter(Imputed==TRUE)

ggplot(temp_all_imputed, aes(Imputed_yearly, Imputed_monthly))+
  geom_point()+
  theme_bw()
# close to 1-1 relationship.
# we have to use this way because the monthly vs. annual models have different stations
# if we just run the annual script and then filter from 1995 onward we lose important
# stations that don't have longer time series and so can't be interpolated before 1995.
```
