...
View(Time_series_imp)
summary(Time_series_imp)
dim(Time_series_imp) # 4680 rows - only 10 NAs remaining
...
dim(chl_table_long) # 3432 rows- with 97 NAs present
...
View(Time_series_imp)
summary(Time_series_imp)
dim(Time_series_imp) # 4680 rows - only 8 NAs remaining
...
dim(amm_table_long) # 3432 rows- with 45 NAs present
...
```{r}
# visualize missing data points - 6 missing nit data points
D28A_na <- nit_table %>% 
  select(Date, D28A) %>% # filter out station of interest
  filter(is.na(D28A)) %>% # find NAs
  mutate(D28A=0) # for visualization purposes set to zero
...
Coefficients:
         ar1    sma1    mean
      0.6418  0.2545  0.3787
s.e.  0.0435  0.0474  0.0209

sigma^2 = 0.0115:  log likelihood = 250.06
AIC=-492.11   AICc=-491.98   BIC=-477.15
...
View(Time_series_imp)
summary(Time_series_imp)
dim(Time_series_imp) # 3432 rows - only 8 NAs remaining in last time steps
...
dim(nit_table_long) # 3432 rows- with 44 NAs present
...
```{r}
# visualize missing data points - 6 missing Phos data points
D28A_na <- Phos_table %>% 
  select(Date, D28A) %>% # filter out station of interest
  filter(is.na(D28A)) %>% # find NAs
  mutate(D28A=0) # for visualization purposes set to zero

plot_D28A <- ggplot(Phos_table, aes(Date, D28A))+
  geom_line()+
  geom_point(data=D28A_na, aes(Date, D28A), pch=4, col="red", size=3)+ # red x = NA
  theme_bw()
plot_D28A
...
```{r}
# time series object using ts - monthly resolution for each year
Phos_D28A <- Phos_table %>% 
  select(Date, D28A) %>% 
  arrange(Date) # single station still for now
Phos_ts <- ts(Phos_D28A$D28A, frequency = 12, start = c(1995,1), end=c(2020,12))
Phos_ts


# fit arima model
Phos_fit <- forecast::auto.arima(Phos_ts, seasonal=TRUE, trace = TRUE, lambda = "auto", stationary=TRUE) #stationary=TRUE for forecasting
# get the same answer with lambda="auto" and unlog data
# if we let stationarity=FALSE, there's a different top model, but it doesn't allow forecasting?
summary(Phos_fit)
```
...
par(mar=c(2,4,1,1))
plot(Phos_D28A$Date,Phos_ts,xlab="Date",ylab="Phos (C)",lwd=2,type="l")
lines(Phos_D28A$Date,Phos_forecast$fitted,col="red")

#Plot predicted versus observed values
plot(Phos_ts,Phos_forecast$fitted,xlab="Observed",ylab="Predicted",pch=".")
abline(0,1,lty=2)
R2 = round(cor.test(Phos_ts,Phos_forecast$fitted,na.rm=T)$estimate^2,2)
mtext(side=3,line=-2,adj=0.1,bquote(R^2 == .(R2)))
...
```{r}
# prep dataframe for for-loop
Phoswodate <- Phos_table %>% 
  arrange(Date)  # ensure it's in chronological order
Phoswodate <- Phoswodate[,-c(1:2)] # then remove date and analyte columns
Phoswodate <- log10(Phoswodate + 1) # add one and log10 transform
summary(Phoswodate)
head(Phoswodate) # 11 cols


### Run this code altogether 
#create an empty matrix
Time_series_inter <- matrix(NA,nrow(Phoswodate),(ncol(Phoswodate))) #create empty matrix
dim(Time_series_inter)
colnames(Time_series_inter) <- colnames(Phoswodate)
head(Time_series_inter)

# Create a vector of years (for each month x year combo = 312)
Year <- lubridate::year(Phos_table$Date)

## start for-loop, make sure to refresh Time_series_inter above
for (i in 1:ncol(Phoswodate)) {
  #create time-series object
  dat_imp <- data.frame(Year, y = Phoswodate[,i])
...
# jpeg(paste0("Imputation_plots/Phos",colnames(Time_series_inter)[i],".jpeg"))
# plot(y_inter,xlab="",ylab="Phos (C)",col="red",main=(Time_series_inter)[i])
# lines(y,col="black")
# mtext(side=3,text=paste("Station #",colnames(Time_series_inter)[i]),line=-1,adj=0.9)
# dev.off()
# 
# 		}else{
# 		#save plot for non-imputed time-series
# 		jpeg(paste0("Imputation_plots/PhosRaw",colnames(Time_series_inter)[i],".jpeg"))
# 		plot(y,xlab="",ylab="Phos (C)",col="black",main=(Time_series_inter)[i])
# 		mtext(side=3,text=paste("Station #",colnames(Time_series_inter)[i]),line=-1,adj=0.9)
# 		dev.off()
		}
    print(i)
...
Coefficients:
      ...
      -0.4764  -0.2842  -0.3129  -0.2923  -0.1861  0.3027  0.2026  0.0000
s.e.   0.0613   0.0615   0.0617   0.0623   0.0588  0.0588  0.0640  0.0011

sigma^2 = 0.0006976:  log likelihood = 679.42
AIC=-1340.85   AICc=-1340.25   BIC=-1307.22

## create new time series with interpolated values
```{r}
#show wide table of imputed values with dates and analyte type
Time_series_imp_wide <- as_tibble(Time_series_inter2) %>% 
  mutate(Date = Phos_table$Date, Analyte= Phos_table$Analyte) %>% # add columns back
  select(Date, Analyte, C10A:P8) # rearrange columns
...
Phos_table_long <- Phos_table %>% 
  ...
  mutate(Missing=ifelse(is.na(Raw_values), TRUE, FALSE)) # Is it missing?
View(Phos_table_long)
summary(Phos_table_long)
dim(Phos_table_long) # 3432 rows- with 49 NAs present

# Join them to create a master table tracking which values were modeled
# Should still be ## NAs that are missing but not imputed
Time_series_imp2 <- Time_series_imp %>% 
  left_join(Phos_table_long) %>% 
  mutate(Imputed=ifelse(Missing==TRUE & Imputed_values>0, TRUE, FALSE))
View(Time_series_imp2)
```
...
ggplot(Time_series_imp2, aes(Date, Imputed_values, color=Imputed))+
  ...
  facet_wrap(Station~., ncol=3, scales = "free_y")+
  scale_color_manual(values=c("gray60", "red", "black"))+
  labs(title="Replacing missing values with ARIMA and Kalman filter", subtitle="Dissolved Orthophosphate")+
  theme_bw()
```
...
## export df
```{r}
Time_series_imp2 %>% write_csv("data/missing_data/imputed_data/diss_orthophos_imputed_data.csv")
```
