...
ARIMA(1,0,3)(2,1,0)[12] with drift (non-stationary) = good interpolation (but no forecasting allowed so skip lines 155-176)
...
## export df
```{r}
df_ts %>% write_csv("data/missing_data/imputed_data/yearly/temperature_imputed_data.csv")
```
...
Date <- tibble(Date=seq(from=lubridate::as_date("1975-01-01"), to=lubridate::as_date("2020-12-01"), by="1 month")) # 312
...
## export df
```{r}
df_ts %>% write_csv("data/missing_data/imputed_data/yearly/chlorophyll_imputed_data.csv")
```
...
## export df
```{r}
df_ts %>% write_csv("data/missing_data/imputed_data/yearly/diss_ammonia_imputed_data.csv")
```
...
## export df
```{r}
df_ts %>% write_csv("data/missing_data/imputed_data/yearly/diss_nitrate_nitrite_imputed_data.csv")
```
...
Secchi_ts <- ts(Secchi_D28A$D28A, frequency = 12, start = c(1975,1), end=c(2020,12))
Secchi_ts
...
for (i in 1:ncol(Secchiwodate)) {
  ...
  y <- ts(dat_imp[2], start= c(1975,1), end = c(2020,12), frequency = 12) # time series object
...
## export df
```{r}
Time_series_imp2 %>% write_csv("data/missing_data/imputed_data/yearly/secchi_imputed_data.csv")
```
...
temp_imputed <- read_csv("data/missing_data/imputed_data/yearly/temperature_imputed_data.csv") %>% 
  mutate(Analyte="Temperature")
chl_imputed <- read_csv("data/missing_data/imputed_data/yearly/chlorophyll_imputed_data.csv")%>% 
  mutate(Analyte="Chlorophyll")
amm_imputed <- read_csv("data/missing_data/imputed_data/yearly/diss_ammonia_imputed_data.csv")%>% 
  mutate(Analyte="DissAmmonia")
nitr_imputed <- read_csv("data/missing_data/imputed_data/yearly/diss_nitrate_nitrite_imputed_data.csv")%>% 
  mutate(Analyte="DissNitrateNitrite")
ophos_imputed <- read_csv("data/missing_data/imputed_data/yearly/diss_orthophos_imputed_data.csv")%>% 
  mutate(Analyte="DissOrthophos")
secchi_imputed <- read_csv("data/missing_data/imputed_data/yearly/secchi_imputed_data.csv")%>% 
...
all_imputed_export_region %>% write_csv("data/missing_data/missing_data/nutrient_data_yearly_regions_imputed.csv")
all_imputed_export_noregions %>% write_csv("data/missing_data/missing_data/nutrient_data_yearly_noregions_imputed.csv")

# 2 - Tanya's model folder
all_imputed_export_region %>% write_csv("data/annual_averages/nutrient_data_yearly_regions_imputed.csv")
all_imputed_export_noregions %>% write_csv("data/annual_averages/nutrient_data_yearly_noregions_imputed.csv")

# double check differences in original vs. imputed
test <- read_csv("data/annual_averages/nutrient_data_yearly_noregions.csv")
```
