...
```{r id-missing-data}
df_wq_fill <- read_wq_data(monthly = TRUE)
...
df_wq_fill <- df_wq_fill %>%
  ...
  select('MonthYear', 'StationID', 'Analyte', 'Value')

df_wq_fill <- df_wq_fill %>% pivot_wider(names_from = 'StationID', values_from = 'Value', values_fn = mean)
...
for (analyte in (unique(df_wq_fill$Analyte))) {
  ...
  write_csv(df, paste0('data/nutrient_missingdat_dfs/missingdat_',analyte,'.csv'))
}
```
...
df_chla <- test %>% filter(Analyte == 'Chlorophyll')
```
