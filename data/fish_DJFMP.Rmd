...
DJFMP_Silverside_seine_monthly_station_fixed<- DJFMP_Silverside_seine
DJFMP_Silverside_seine_monthly_station_fixed$Station<-factor(DJFMP_Silverside_seine_monthly_station_fixed$Station)
levels(DJFMP_Silverside_seine_monthly_station_fixed$Station)[levels(DJFMP_Silverside_seine_monthly_station_fixed$Station)=="SR012W"] <- "SR012E"
levels(DJFMP_Silverside_seine_monthly_station_fixed$Station)[levels(DJFMP_Silverside_seine_monthly_station_fixed$Station)=="SJ058E"] <- "SJ058W"


DJFMP_Silverside_seine_monthly_region<- DJFMP_Silverside_seine_monthly_station_fixed %>% mutate(Month=month(Date),Year=year(Date),Biomass=ifelse(is.na(Length),0, (0.0097*(Length^2.87))*Count)) %>%
  ...
  group_by(Year,Month, Station, Datetime, Region) %>% summarise(Catch_per_seine=sum(Count),Biomass=sum(Biomass)) %>%
  group_by(Year,Month, Station, Region) %>% summarise(Catch_per_seine=mean(Catch_per_seine),Biomass=mean(Biomass)) %>%
  group_by(Year,Month, Region) %>% summarise(DJFMP_fish_catch_per_seine_Silverside=mean(Catch_per_seine),DJFMP_fish_biomass_Silverside=mean(Biomass))

write.csv(DJFMP_Silverside_seine_monthly_region,row.names=FALSE,file=file.path("data/monthly_averages/fish_DJFMP_monthly.csv"))
