...
write.csv(LT_clams_year_wide, "data/annual_averages/clams_annual_noregions.csv")
...
write.csv(LT_amph_year_wide, "data/annual_averages/amphipod_annual_noregions.csv")
...
* Assign annual and monthly stations to regions
* Plot to make sure all stations fall into a region
* Add region assignments to benthic file for monthly and annual analysis
...
ben_st_reg= ben_ST %>%
  region_assigner(analysis="monthly") #Stations C9 and P8 are dropped due to being outside the south region

```

## Calculating CPUE with regions assigned

* Recalculate CPUE for 
* 1. Clam and Amphipod Annual w/o Regions (already done earlier) 
...
#clam annual CPUE with region

Reg_clams_year <- ben_LT_Reg %>% 
  group_by(Genus, Year, Region) %>%
  summarize(Orgs_Total = sum(Count, na.rm = TRUE), .groups = "drop") %>% 
  left_join(LT_grabs_year) %>% 
  mutate(CPUE_Total = (Orgs_Total/Grab_Count)/0.052) %>% 
  filter(Genus %in% c("Potamocorbula", "Corbicula"))

reg_clams_year_wide <- Reg_clams_year %>% 
  select(-Orgs_Total, -Grab_Count) %>% 
  pivot_wider(names_from = Genus, values_from = CPUE_Total) %>% 
  rename(Corbicula_cpue = Corbicula,
         Potamocorbula_cpue = Potamocorbula)

write.csv(reg_clams_year_wide, "data/annual_averages/clams_annual_regions.csv")

#amphipod annual with regions

reg_amph_year <- ben_LT_Reg %>% 
  group_by(Order_level, Year, Region) %>%
  summarize(Orgs_Total = sum(Count, na.rm = TRUE), .groups = "drop") %>% 
  left_join(LT_grabs_year) %>% 
  mutate(CPUE_Total = (Orgs_Total/Grab_Count)/0.052) %>% 
  filter(Order_level == "Amphipoda")

reg_amph_year_wide <- reg_amph_year %>% 
  select(-Orgs_Total, -Grab_Count) %>% 
  pivot_wider(names_from = Order_level, values_from = CPUE_Total) %>% 
  rename(Amphipoda_cpue = Amphipoda)

write.csv(reg_amph_year_wide, "data/annual_averages/amphipod_annual_regions.csv")


#clam monthly with region

reg_clams_mon <- ben_st_reg %>% 
  mutate(Date = paste(Month, Year)) %>% 
  group_by(Genus,Date, Region) %>%
  summarize(Orgs_Total = sum(Count, na.rm = TRUE), .groups = "drop") %>% 
  left_join(ST_grabs_month) %>% 
  mutate(CPUE_Total = (Orgs_Total/Grab_Count)/0.052) %>% 
  filter(Genus %in% c("Potamocorbula", "Corbicula"))

reg_clams_mon_wide <- reg_clams_mon %>% 
  select(-Orgs_Total, -Grab_Count) %>% 
  pivot_wider(names_from = Genus, values_from = CPUE_Total) %>% 
  rename(Corbicula_cpue = Corbicula,
         Potamocorbula_cpue = Potamocorbula)

write.csv(ST_clams_mon_wide, "data/monthly_averages/clams_monthly_regions.csv")


#amphipod monthly with regions

reg_amph_mon <- ben_st_reg %>% 
  mutate(Date = paste(Month, Year)) %>% 
  group_by(Order_level,Date, Region) %>%
  summarize(Orgs_Total = sum(Count, na.rm = TRUE), .groups = "drop") %>% 
  left_join(ST_grabs_month) %>% 
  mutate(CPUE_Total = (Orgs_Total/Grab_Count)/0.052) %>% 
  filter(Order_level == "Amphipoda")

reg_amph_mon_wide <- reg_amph_mon %>%
  select(-Orgs_Total, -Grab_Count) %>% 
  pivot_wider(names_from = Order_level, values_from = CPUE_Total) %>% 
  rename(Amphipod_cpue = Amphipoda)

write.csv(reg_amph_mon_wide, "data/monthly_averages/amphipod_monthly_regions.csv")
