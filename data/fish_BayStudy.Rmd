...
```{r}
# look at Brian's Bay Study full dataframe from Line 95
View(BayStudy_full)
...
BayStudy_full_monthly <- BayStudy_full %>% 
  ...
  filter(Station > 300) %>% #remove central and south bay stations
  mutate(Monthly=TRUE)
...
map3 <- ggmap(map_obj) +
  theme_void() +
  geom_point(data = BayStudy_full_monthly, aes(x = Longitude, y = Latitude), shape = 21, size = 3) +
  geom_text(data = BayStudy_full_monthly, aes(label = Station, x = Longitude, y = Latitude), vjust = 0, hjust = 0, size=2.5)+
  labs(fill=NULL)
map3
