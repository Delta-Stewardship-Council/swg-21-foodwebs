...
# read in absolute fp
abs_path <- function(fp_rel = NULL, org = NULL) {
  # set org fp
  if (org == 'dwr'){
...
# Plotting and selecting just the continuously sampled sites from the mid 80s and 90s

Black: non-continuous stations, Green: continuous since 1980s, Blue: continuous since 1990s 
```{r}
sites_cont <- sites %>% 
  filter(StationClean %in% c("D7", "D28A", "D4", "P8", "D6", "D41", "D41A", "D24", "D16", "C9"))%>%
  mutate(Start=c(rep("1980s", 3), rep("1990s", 7)))
...
leaflet(sites) %>% 
  ...
  addCircleMarkers(data = sites,
                         ...
                         color = "black",
                         label = ~StationClean) %>% 
  addCircleMarkers(data = sites_cont,
                         lat = ~Latitude,
                         lng = ~Longitude,
                         radius = ~(n_months/50), # arbitrary scaling
                         fillColor = ~colorFactor(palette=c("chartreuse4", "dodgerblue3"), domain=unique(sites_cont$Start))(Start),
                         fillOpacity = 1,
                         weight = 0.25,
                         color = "black",
                         label = ~StationClean)
...
Export stations for regions
```{r}
sites_cont%>%
  st_drop_geometry()%>%
  select(Station=StationClean, Latitude, Longitude)%>%
  write_csv("data/stations/stations_benthic.csv")
```
...
```{r}
write_csv(ben_clams_year_wide, "data/annual_averages/benthic.csv")
```
