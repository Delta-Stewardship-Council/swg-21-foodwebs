...
Check if Ammonia Paradox is observed with temporal/spatial averaging.

```{r message=FALSE, warning = FALSE, results = 'hide'}
empwq_annual_mean_wide$DissAmmonia_uM <- empwq_annual_mean_wide$DissAmmonia*100*0.5872

plt <- ggplot(empwq_annual_mean_wide, aes(DissAmmonia_uM, Chlorophyll)) +
  geom_point() +
...
Doesn't seem to be, but still see it when looking at raw (monthly, by station) data:
...
plt <- ggplot(df_wq_wide, aes(DissAmmonia_uM, Chlorophyll)) +
  geom_point() +
  geom_vline(xintercept = 4)
  theme(axis.text.x = element_text(angle = 90))
